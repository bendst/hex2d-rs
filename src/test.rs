...
fn with_test_points<F: Fn(Coordinate<i32>) -> ()>(f: F) {
...
fn direction_add_and_sub() {
    ...
    with_test_points(|c| for &sd in Direction::all() {
...
#[test]
fn range_distance() {
    with_test_points(|c| for r in 0..10 {
...
fn simple_to_pixel() {
    ...
    let c: Coordinate<i32> = (2i32, -1i32).into();
    assert_eq!(c.to_pixel_float(f_spacing), (6f32, 0f32));

    let c: Coordinate<i32> = (-2i32, 1i32).into();
    assert_eq!(c.to_pixel_float(f_spacing), (-6f32, 0f32));


    let c: Coordinate<i32> = (1i32, 1i32).into();
    assert_eq!(c.to_pixel_float(p_spacing), (0f32, -6f32));

    let c: Coordinate<i32> = (2i32, 2i32).into();
    assert_eq!(c.to_pixel_float(p_spacing), (0f32, -12f32));
}
...
fn simple_rotations_around_zero() {
    with_test_points(|c| {
        ...
        assert_eq!(
            c.rotate_around_zero(LeftBack),
            c.rotate_around_zero(Left).rotate_around_zero(Left)
        );
        assert_eq!(
            c.rotate_around_zero(RightBack),
        ...
        assert_eq!(c.rotate_around_zero(Back), -c);
    });
}
...
fn simple_direction_from_center() {
    ...
    for &dir in Direction::all().iter() {
        ...
        assert_eq!(
            (c + dir + (dir + Left)).direction_from_center_cw(),
            Some(dir)
        );
        assert_eq!(
            (c + dir + (dir + Right)).direction_from_center_ccw(),
            Some(dir)
        );
    }
}
...
fn simple_direction_to() {
    ...
    with_test_points(|c| {
        ...
        for &dir in Direction::all().iter() {
            ...
            assert_eq!(
                c.direction_to_cw(c + dir + (dir + Left) + dir + (dir + Left)),
                Some(dir)
            );
            assert_eq!(
                c.direction_to_ccw(c + dir + (dir + Right) + dir + (dir + Right)),
                Some(dir)
            );
        }
    });
